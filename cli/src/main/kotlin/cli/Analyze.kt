...
class Analyze : CliktCommand(help = "Analyze PrintScript file") {
    ...
    override fun run() {
        ...
        if (config == null) {
            ...
            runner.runAnalyze(FileInputStream(file), FileInputStream(configFile), CliErrorHandler())
        } else {
            runner.runAnalyze(FileInputStream(file), FileInputStream(config!!), CliErrorHandler())
        }
    }
}
